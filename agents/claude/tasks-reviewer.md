...
- every issue created by this Tasks transaction has structured metadata containing exact key/value `"tasks-skill": "owned"`
- no pre-existing/reused issue has been given the `tasks-skill` ownership marker merely because Tasks reused or updated it
