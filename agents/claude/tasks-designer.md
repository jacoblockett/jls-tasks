...
If a current tracker condition prevents faithful application but can be resolved without new user authority or a new product decision, return REPAIR_REQUIRED rather than BLOCKED. REPAIR_REQUIRED must be returned before the first mutation, with CREATED/UPDATED empty, so the parent can repair and re-review the design without leaving an unreviewed partial graph.
Once the first mutation occurs, finish the reviewed application and readback unless a genuine external backend/tool failure prevents completion. Preserve any durable mapping already created if that happens.
...
- DEFICIENCY_ID: <D...>
  RESULT: RESOLVED | EXTERNAL_BLOCKER
  STRATEGY: <materially specific durable-state repair approach used this round>
